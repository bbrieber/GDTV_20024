...
func init()->void:
	run_acceleration  = player.player_movement_config.run_velocity / player.player_movement_config.run_time_to_peak
	run_deceleration = (-player.player_movement_config.run_velocity) / player.player_movement_config.run_time_to_stop
...
func _get_stop_velocity(direction: float,delta:float) ->float:
	...
	var current_dir_sign : int = 1 if player.velocity.x > 0 else -1
